...
class WebSocketService {
  ...
  private async resolveBackendUrl(): Promise<string> {
    try {
      ...
      // Esempio: http://192.168.123.160:3000/api/v1
      let protocol = 'ws';
      let host = '';
      ...
      if (match) {
        ...
        host = '192.168.123.160:3000';
      }
      
      // 2. Costruisci l'URL WebSocket
      // Nota: il percorso corretto secondo il server è "/api/notifications/ws"
      // come specificato nel log del server nell'init
  ...
  private handleError(event: Event): void {
    ...
    if (errorMessage.includes('400 Bad Request')) {
      ...
      // Se riceviamo più volte errori 400, potrebbe esserci un problema con il token
