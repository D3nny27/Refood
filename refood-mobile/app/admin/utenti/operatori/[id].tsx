...
const GestioneOperatoriScreen = () => {
  ...
  const loadOperatori = async () => {
    try {
      ...
      // Carica gli operatori associati - Nota: getUtentiAssociati non accetta utenteId come parametro
      // Quindi usiamo getUtenti con il filtro associatiA
      const response = await utentiService.getUtenti({ associatiA: utenteId });
      
      if (response && response.data) {
        setOperatori(response.data);
        setFilteredOperatori(response.data);
      } else {
        Toast.show({
          type: 'error',
          text1: 'Errore',
  ...
  const addOperatore = async () => {
    ...
    try {
      ...
      try {
        ...
        const response = await utentiService.getUtenti({ nome: newOperatorEmail });
        const userResponse = response.data.find(u => u.email === newOperatorEmail);
