...
exports.getProfile = async (req, res, next) => {
  try {
    ...
    // Ottieni i centri associati all'attore
...
exports.getAllAttori = async (req, res, next) => {
  try {
    ...
    let queryBase = `
      ...
      FROM Attori u
      LEFT JOIN AttoriCentri uc1 ON u.id = uc1.attore_id
      WHERE 1=1
    `;
    ...
    // 1. Attori associati ai centri dell'amministratore 
    // 2. Attori creati dall'amministratore
    // 3. L'amministratore stesso
    conditions.push(`
      (
        EXISTS (
          SELECT 1 FROM AttoriCentri uc_admin
          WHERE uc_admin.attore_id = ?
          AND EXISTS (
            SELECT 1 FROM AttoriCentri uc_user
    ...
    // Array per memorizzare gli attori finali con i centri
    const attoriWithCentri = [];
    
    // Per ogni attore, recupera i centri associati
    for (const attore of attori) {
      try {
        ...
        attoriWithCentri.push({ ...attore, centri });
      } catch (err) {
        logger.error(`Errore nel recupero dei centri per l'attore ${attore.id}: ${err.message}`);
        // Aggiungiamo comunque l'attore, ma con un array vuoto di centri
        attoriWithCentri.push({ ...attore, centri: [] });
      }
    }
